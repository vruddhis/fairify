...
''' Base class '''
class LLM():
    def __init__(self, nout_per_prompt, max_tokens_per_prompt): #sets number of outputs per prompt and max tokens to generate in the output
    ...
    def generate(self, prompts, wrap_by_input=False, **kwargs):
        ...
        #so every response is (prompt, output, score)
        if wrap_by_input:
            n = len(prompts)
